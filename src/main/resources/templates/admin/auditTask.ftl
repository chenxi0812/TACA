			...
			<form class="am-form am-g">
				<table width="100%"
					...
					<tbody>
						...
						<tr>
							<td>${auditTask.id}</td>
							<td title="${auditTask.feedBackWay}"><a
								href="${base}/admin/task/taskInfo/${auditTask.taskId}">${auditTask.taskId}</a></td>
							<td class="am-hide-sm-only">${auditTask.name}</td>
							<td class="am-hide-sm-only">${auditTask.preStarNumber}</td>
							<td class="am-hide-sm-only"><input type="number" name="actStarNumber"
								style="width: 70px; height: 27px;"
								value="${auditTask.preStarNumber}" id="actStar${auditTask.id}"></td>
							<td class="am-hide-sm-only">${auditTask.textArea}</td>
							<td class="am-hide-sm-only"><a
								href="${base}/admin/audit/download?fileAddress=${auditTask.fileAddress}">下载</a></td>
